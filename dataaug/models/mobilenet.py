    ...
    def forward(self, x):
        ...
        if self.stride == 1 and action == 1:
            ...
            if self.bdp > 0:
                ...
                bdp_mask = (
                    torch.bernoulli(self.bdp * torch.ones(1, out.size(1), out.size(2), out.size(3)).to(out.device))
                    / self.bdp
                )
                out = bdp_mask * out
...
class MobileNetV2(nn.Module):
    ...
    def __init__(self, num_classes=10, num_channels=3, train_dp=0, test_dp=0, droplayer=0, bdp=0):
