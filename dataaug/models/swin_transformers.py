"""SWIN transformer implementation from the microsoft repo."""
...
#
#     # ughhhhhh
#     kernel_path = os.path.abspath(os.path.join("..", "Swin-Transformer"))
...
#     print("git clone https://github.com/microsoft/Swin-Transformer.git")
#     print("cd kernels/window_process/")
...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        # cyclic shift
        if self.shift_size > 0:
            if not self.fused_window_process or (x.device == torch.device("cpu")):
...
class WindowAttention_v2(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, attn_drop=0.0, proj_drop=0.0, pretrained_window_size=[0, 0]):
        ...
        relative_coords_table = (
            torch.stack(torch.meshgrid([relative_coords_h, relative_coords_w])).permute(1, 2, 0).contiguous().unsqueeze(0)
        )  # 1, 2*Wh-1, 2*Ww-1, 2
        ...
        **kwargs,
        ...
        for i_layer in range(self.num_layers):
            layer = BasicLayer_v2(
                ...
                drop_path=dpr[sum(depths[:i_layer]) : sum(depths[: i_layer + 1])],
                norm_layer=norm_layer,
                downsample=PatchMerging_v2 if (i_layer < self.num_layers - 1) else None,
                use_checkpoint=use_checkpoint,
