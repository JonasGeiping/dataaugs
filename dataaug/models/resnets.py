...
def resnet_depths_to_config(depth):
    if depth == 8:
        ...
        raise ValueError("Depth not registered.")
    return block, layers
    ...
    def __init__(
        ...
        if norm == "skipinit":
            ...
            if block is BasicBlock:
                block = BasicBlockSkipInit
            elif block is Bottleneck:
                block = BottleneckSkipInit
            downsample = f"preact-{downsample}"
        else:
            self.use_bias = False
        ...
        if stem == "CIFAR":
            conv1 = self._conv_layer(channels, self.inplanes, kernel_size=3, stride=1, padding=1, groups=1, bias=self.use_bias, dilation=1)
            ...
            conv1 = self._conv_layer(channels, stem_width, kernel_size=3, stride=2, padding=1, groups=1, bias=self.use_bias, dilation=1)
            bn1 = self._norm_layer(stem_width)
            conv2 = self._conv_layer(stem_width, stem_width, kernel_size=3, stride=1, padding=1, groups=1, bias=self.use_bias, dilation=1)
...
class BasicBlockSkipInit(torch.nn.Module):
    expansion = 1
...
class Bottleneck(torch.nn.Module):
    ...
    def __init__(
        ...
        self.conv2 = conv(width, width, kernel_size=3, stride=stride, padding=dilation, groups=groups, bias=bias, dilation=dilation)
...
class BottleneckSkipInit(torch.nn.Module):
    # Bottleneck in torchvision places the stride for downsampling at 3x3 convolution(self.conv2)
    # while original implementation places the stride at the first 1x1 convolution(self.conv1)
    # according to "Deep residual learning for image recognition"https://arxiv.org/abs/1512.03385.
    # This variant is also known as ResNet V1.5 and improves accuracy according to
